...
if(isset($data->id)){
    ...
    $check_post = "SELECT * FROM `jadwal_kuliah` WHERE id_matakuliah=:id";
    $check_post_stmt = $conn->prepare($check_post);
    $check_post_stmt->bindValue(':id', $post_id,PDO::PARAM_STR);
    ...
    if($check_post_stmt->rowCount() > 0){
        ...
        $delete_post = "DELETE FROM `jadwal_kuliah` WHERE id_matakuliah=:id";
        $delete_post_stmt = $conn->prepare($delete_post);
        $delete_post_stmt->bindValue(':id', $post_id,PDO::PARAM_STR);
